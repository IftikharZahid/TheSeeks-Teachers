...
files.forEach(file => {
  ...
  const handleDeleteExam`;
        ...
        content = content.replace(/\n    \}\n  \};\n\n  const handleDeleteExam/g, wrapper);
        modified = true;
    }

    // 3. Update header Save button
    if (content.includes('<Text style={{ fontSize: scale(12), fontWeight: \'700\', color: \'#fff\' }}>Save</Text>')) {
        content = content.replace(
        ...
        content = content.replace(
            /<Ionicons name="checkmark" size=\{16\} color=\{isDark \? theme.text : '#fff'\} \/>\s*<Text style=\{\{ fontSize: scale\(12\), fontWeight: '700', color: '#fff' \}\}>Save<\/Text>/g,
    ...
    // 4. Update main Save button (fsFormSaveButton)
    if (content.includes('<Text style={styles.fsFormSaveButtonText}>{editingExam ? \'Update Record\' : \'Save Record\'}</Text>')) {
        content = content.replace(
        ...
        content = content.replace(
            /<Ionicons name=\{editingExam \? 'checkmark-circle' : 'save-outline'\} size=\{18\} color=\{isDark \? theme\.text : '#fff'\} \/>\s*<Text style=\{styles\.fsFormSaveButtonText\}>\{editingExam \? 'Update Record' : 'Save Record'\}<\/Text>/g,
    ...
    if (modified) {
        ...
        console.log('Added spinner to', file);
    } else {
        console.log('No modifications needed or target not found in', file);
    }
});
