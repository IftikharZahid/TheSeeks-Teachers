  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]} edges={['top', 'left', 'right']}>
      ...
      {creatingAccounts && acctTotal > 0 && (
        <View style={{ paddingHorizontal: scale(16), paddingVertical: scale(10), backgroundColor: theme.card, borderBottomWidth: 1, borderBottomColor: theme.border }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: scale(6) }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: scale(6) }}>
              <ActivityIndicator size="small" color="#f59e0b" />
              <Text style={{ fontSize: scale(13), fontWeight: '700', color: theme.text }}>Creating Accounts...</Text>
            </View>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: scale(10) }}>
              ...
              >
                <Text style={{ fontSize: scale(11), fontWeight: '700', color: theme.error }}>Stop</Text>
              </TouchableOpacity>
            </View>
          </View>
          <View style={{ height: scale(6), borderRadius: scale(3), backgroundColor: theme.border + '40', overflow: 'hidden' }}>
            <View style={{ height: '100%', borderRadius: scale(3), backgroundColor: '#f59e0b', width: `${acctTotal > 0 ? Math.min((acctProgress / acctTotal) * 100, 100) : 0}%` }} />
          </View>
      ...
      {loading ? (
        <View style={styles.skeletonContainer}>
          ...
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <View key={i} style={[styles.skeletonCard, { backgroundColor: theme.card }]}>
              {/* Serial Number */}
              <SkeletonBlock width={22} height={22} borderRadius={11} theme={theme} style={{ marginRight: scale(8) }} />
              {/* Avatar */}
              <SkeletonBlock width={44} height={44} borderRadius={22} theme={theme} style={{ marginRight: scale(10) }} />
              {/* Text Lines */}
              <View style={{ flex: 1 }}>
                <SkeletonBlock width="70%" height={12} borderRadius={4} theme={theme} style={{ marginBottom: scale(6) }} />
                <SkeletonBlock width="50%" height={9} borderRadius={3} theme={theme} style={{ marginBottom: scale(4) }} />
                <SkeletonBlock width="40%" height={9} borderRadius={3} theme={theme} />
              </View>
              {/* Action Buttons */}
              <View style={{ flexDirection: 'row', gap: scale(10), marginLeft: scale(8) }}>
        ...
        <>
          ...
          <View style={[styles.tableHeader, { backgroundColor: theme.card, borderBottomColor: theme.border }]}>
            <Text style={[styles.tableHeaderCell, { width: scale(26), color: theme.textSecondary }]}>#</Text>
            ...
            <Text style={[styles.tableHeaderCell, { width: scale(56), textAlign: 'right', color: theme.textSecondary }]}>ACTIONS</Text>
          </View>
          <FlatList
            ...
            renderItem={({ item, index }) => {
              ...
              return (
                ...
                >
                  {/* Serial */}
                  <Text style={{ width: scale(26), fontSize: scale(10), color: theme.textSecondary, fontWeight: '600' }}>
                    {index + 1}
                  </Text>
                  {/* Avatar + Name/Father */}
                  <View style={{ flex: 2.2, flexDirection: 'row', alignItems: 'center', gap: scale(7) }}>
                    <View style={[styles.tableAvatar, { backgroundColor: theme.primary + '18' }]}>
                      {item.profileImage ? (
                        <Image source={{ uri: item.profileImage }} style={{ width: scale(26), height: scale(26), borderRadius: scale(13) }} />
                      ) : (
                        <Text style={{ fontSize: scale(10), fontWeight: '800', color: theme.primary }}>{initials}</Text>
                      )}
                    </View>
                    <View style={{ flex: 1 }}>
                  ...
                  {/* Class */}
                  <Text style={{ flex: 1, fontSize: scale(11), color: theme.textSecondary, textAlign: 'center' }} numberOfLines={1}>
                    {item.grade || '—'}
                  </Text>
                  {/* Student ID */}
                  <Text style={{ flex: 1.4, fontSize: scale(10), color: theme.textSecondary, textAlign: 'center', fontFamily: 'monospace' }} numberOfLines={1}>
                    {item.studentId || '—'}
                  </Text>
                  {/* Actions */}
                  <View style={{ width: scale(56), flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', gap: scale(8) }}>
                    <TouchableOpacity
                      onPress={() => openModal(item)}
          ...
          />
          {/* Count footer */}
          <View style={{ paddingHorizontal: scale(12), paddingVertical: scale(5), borderTopWidth: 1, borderTopColor: theme.border }}>
            <Text style={{ fontSize: scale(10), color: theme.textSecondary, textAlign: 'center' }}>
      ...
      <Modal visible={modalVisible} animationType="slide" statusBarTranslucent>
        <View style={[styles.fullScreenModal, { backgroundColor: theme.background }]}>
          ...
          >
            ...
            <View style={[styles.cFormSection, { backgroundColor: theme.background, borderColor: theme.border }]}>
              ...
              <View style={styles.cFormRow}>
                <View style={styles.cFormCol}>
                  ...
                  >
                    <Text style={{ flex: 1, fontSize: scale(13), color: grade ? theme.text : theme.textSecondary }} numberOfLines={1}>
                      {grade || 'Select'}
                    </Text>
                ...
                <View style={styles.cFormCol}>
                  ...
                  <TouchableOpacity
                    ...
                    style={[styles.cFormInput, { backgroundColor: theme.card, borderColor: theme.border, flexDirection: 'row', alignItems: 'center' }]}
                  >
                    <Text style={{ flex: 1, fontSize: scale(13), color: gender ? theme.text : theme.textSecondary }} numberOfLines={1}>
                      {gender || 'Select'}
                    </Text>
                    <Ionicons name="chevron-down" size={14} color={theme.textSecondary} />
                  </TouchableOpacity>
            ...
            {editingStudent && (
              <View style={[styles.cFormSection, { backgroundColor: theme.background, borderColor: theme.border }]}>
                <View style={styles.cFormSectionHeader}>
                  ...
                  <View style={{ marginLeft: 'auto', flexDirection: 'row', alignItems: 'center', backgroundColor: theme.primary + '12', paddingHorizontal: scale(8), paddingVertical: scale(2), borderRadius: scale(8) }}>
                    <Ionicons name="lock-closed" size={9} color={theme.primary} style={{ marginRight: scale(3) }} />
                    <Text style={{ fontSize: scale(9), fontWeight: '700', color: theme.primary, textTransform: 'uppercase' }}>Read Only</Text>
                  </View>
                </View>
                <View style={{ backgroundColor: theme.card, borderRadius: scale(10), borderWidth: 1, borderColor: theme.border, overflow: 'hidden' }}>
                  {/* ID */}
                  <View style={{ flexDirection: 'row', alignItems: 'center', padding: scale(12), borderBottomWidth: 1, borderBottomColor: theme.border }}>
                    <Ionicons name="id-card-outline" size={18} color="#6366f1" style={{ width: scale(28) }} />
                    <Text style={{ fontSize: scale(12), color: theme.textSecondary, width: scale(75), fontWeight: '500' }}>Student ID</Text>
                    <Text style={{ flex: 1, fontSize: scale(13), color: theme.text, fontFamily: 'monospace', fontWeight: '600' }} numberOfLines={1}>{studentId}</Text>
                    <TouchableOpacity 
                      onPress={async () => { await Clipboard.setStringAsync(studentId); Alert.alert('Copied', 'Student ID copied to clipboard'); }} 
                      style={{ padding: scale(6), backgroundColor: theme.primary + '10', borderRadius: scale(6) }}
                    >
                  ...
                  {/* Email */}
                  <View style={{ flexDirection: 'row', alignItems: 'center', padding: scale(12), borderBottomWidth: 1, borderBottomColor: theme.border }}>
                    <Ionicons name="mail-outline" size={18} color="#10b981" style={{ width: scale(28) }} />
                    <Text style={{ fontSize: scale(12), color: theme.textSecondary, width: scale(75), fontWeight: '500' }}>Email</Text>
                    <Text style={{ flex: 1, fontSize: scale(13), color: theme.text, fontWeight: '500' }} numberOfLines={1}>{email}</Text>
                    <TouchableOpacity 
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', padding: scale(12) }}>
                    <Ionicons name="key-outline" size={18} color="#f59e0b" style={{ width: scale(28) }} />
                    <Text style={{ fontSize: scale(12), color: theme.textSecondary, width: scale(75), fontWeight: '500' }}>Password</Text>
                    <Text style={{ flex: 1, fontSize: scale(13), color: theme.text, fontFamily: 'monospace', fontWeight: '600' }} numberOfLines={1}>{password || 'Not Set'}</Text>
                    <TouchableOpacity 
                ...
                <TouchableOpacity 
                  ...
                  style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: theme.primary + '15', borderRadius: scale(8), paddingVertical: scale(10), marginTop: scale(12) }}
                >
                  <Ionicons name="copy" size={16} color={theme.primary} style={{ marginRight: scale(6) }} />
                  <Text style={{ fontSize: scale(13), fontWeight: '700', color: theme.primary }}>Copy All Credentials</Text>
                </TouchableOpacity>
              </View>
            )}
            ...
            {/* Save Button */}
            <TouchableOpacity onPress={handleSaveStudent} activeOpacity={0.85} style={{ marginTop: scale(12) }}>
              <LinearGradient
                colors={['#667eea', '#764ba2']}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 0 }}
          ...
          <View style={[styles.viewModalContent, { backgroundColor: theme.card }]}>
            ...
            <View style={styles.floatingProfileSection}>
              <View style={styles.modalHeaderRow}>
                ...
                <View style={[styles.profileImageContainer, { width: scale(90), height: scale(90), borderRadius: scale(45), borderWidth: 4, borderColor: '#ffffff', backgroundColor: theme.card, marginTop: scale(-35), elevation: 5, shadowColor: '#000', shadowOffset: { width: 0, height: scale(4) }, shadowOpacity: 0.2, shadowRadius: 5 }]}>
                  {viewingStudent?.profileImage ? (
                    <Image
                      source={{ uri: viewingStudent.profileImage }}
                      style={{ width: '100%', height: '100%', borderRadius: scale(45) }}
                    />
                  ) : (
      ...
      <Modal visible={uploading} animationType="fade" transparent>
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.65)', justifyContent: 'center', alignItems: 'center', paddingHorizontal: scale(32) }}>
          <View style={{ backgroundColor: theme.card, borderRadius: scale(20), padding: scale(28), width: '100%', alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.25, shadowRadius: 20, elevation: 10 }}>
            <View style={{ width: scale(56), height: scale(56), borderRadius: scale(28), backgroundColor: theme.primary + '15', justifyContent: 'center', alignItems: 'center', marginBottom: scale(16) }}>
              <Ionicons name="cloud-upload" size={28} color={theme.primary} />
            </View>
            <Text style={{ fontSize: scale(17), fontWeight: '700', color: theme.text, marginBottom: scale(4) }}>Uploading Students</Text>
            <Text style={{ fontSize: scale(12), color: theme.textSecondary, marginBottom: scale(20) }}>
              {uploadTotal > 0 ? `${uploadProgress} / ${uploadTotal} students` : 'Parsing file...'}
            </Text>

            {/* Progress bar */}
            <View style={{ width: '100%', height: scale(8), backgroundColor: isDark ? '#1e293b' : '#e2e8f0', borderRadius: scale(4), overflow: 'hidden', marginBottom: scale(12) }}>
              <View
                style={{
                  height: '100%',
            ...
            {/* Percentage */}
            <Text style={{ fontSize: scale(24), fontWeight: '800', color: theme.primary }}>
      ...
      <Modal visible={choiceModalVisible} animationType="fade" transparent>
        ...
        >
          <View style={[styles.modalContent, { backgroundColor: theme.card, alignItems: 'center', paddingHorizontal: scale(20), paddingVertical: scale(24) }]}>
            ...
            <Text style={[styles.modalTitle, { color: theme.text, marginTop: scale(12), marginBottom: scale(6) }]}>Add New Student</Text>
            <Text style={{ color: theme.textSecondary, marginBottom: scale(20), textAlign: 'center', fontSize: scale(13), lineHeight: 18 }}>
            ...
            <TouchableOpacity
              onPress={() => setChoiceModalVisible(false)}
              style={{ marginTop: scale(12), padding: scale(8) }}
            >
              <Text style={{ color: theme.textSecondary, fontSize: scale(14), fontWeight: '600' }}>Cancel</Text>
            </TouchableOpacity>
          </View>
        </TouchableOpacity>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  content: { padding: scale(12), paddingTop: scale(8) },
  ...
  details: { fontSize: scale(10), marginBottom: scale(1) },
  cardActions: { flexDirection: 'row', gap: scale(12), paddingLeft: scale(8), alignItems: 'center' },
  ...
  // Table Styles
  tableContent: { paddingBottom: scale(4) },
  tableHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    paddingHorizontal: scale(12),
