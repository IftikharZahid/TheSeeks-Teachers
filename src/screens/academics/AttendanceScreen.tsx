...
const STATUS_META: Record<StatusType, { label: string; color: string; icon: string; short: string }> = {
  ...
  pending: { label: 'Pending', color: '#94a3b8', icon: 'refresh',          short: 'C' },
};
...
export const AdminAttendanceScreen: React.FC = () => {
  ...
  const isAdminDataLoaded = useAppSelector(s => s.attendance.isAdminDataLoaded);
  ...
  const filters = useAppSelector(s => s.attendance.filters);
  const classFilter = filters.classFilter;
  const genderFilter = filters.genderFilter;
  const viewYear = filters.viewYear;
  const viewMonth = filters.viewMonth;
  const selectedDate = filters.selectedDate ?? todayStr;

  const [visibleCount, setVisibleCount] = useState(200);

  // Sync route param
  useEffect(() => {
    if (route.params?.selectedClass && route.params.selectedClass !== classFilter) {
      dispatch(setAttendanceFilters({ classFilter: route.params.selectedClass }));
    }
  }, [route.params?.selectedClass]);

  const setGenderFilter = (val: string) => dispatch(setAttendanceFilters({ genderFilter: val }));
  const setSelectedDate = (val: string) => dispatch(setAttendanceFilters({ selectedDate: val }));
  ...
  const onRefresh = useCallback(async () => {
    ...
    setRefreshing(true);
    try {
      await dispatch(fetchAllAttendanceRecords()).unwrap();
    } catch (e) {
  ...
  /* Init students from Redux — resolve Firebase Auth UID in background */
  useEffect(() => {
    if (loadingStudents || studentsRaw.length === 0) return;

    // 1. Instantly populate students array
    const buildRows = (emailMap: Record<string, string>) => {
      return studentsRaw.map(s => {
        const resolvedUid = s.uid?.trim() || s.authUid?.trim() || 
                            (s.email && emailMap[(s.email || '').toLowerCase()]) || 
                            s.id;
        return {
    ...
    setStudents(buildRows({})); // Render instantly!

    // 2. Only if needed, do the slow lookup
    const studentsNeedingLookup = studentsRaw.filter(s => !(s.uid || s.authUid) && s.email);
    if (studentsNeedingLookup.length > 0) {
      (async () => {
        ...
        try {
          ...
          setStudents(buildRows(emailToUid));
        } catch (e) {
  ...
  }, [students, dispatch, isAdminDataLoaded]);
  ...
  const quickUpdate = useCallback((studentId: string, status: StatusType) => {
    ...
    const timerKey = `${studentId}_${selectedDate}`;
    clearTimeout(writeTimers.current[timerKey]);
    writeTimers.current[timerKey] = setTimeout(() => {
      dispatch(writeStudentAttendance({ studentId }));
    }, 600);
  ...
  const prevWeek = () => {
    ...
    const newStr = fmt(nextDate);
    dispatch(setAttendanceFilters({ 
        selectedDate: newStr,
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [students, classFilter, genderFilter]);
  ...
  const markAll = useCallback((status: StatusType) => {
    ...
    filtered.forEach(s => {
      dispatch(writeStudentAttendance({ studentId: s.studentId }));
    });
  }, [selectedDate, filtered, dispatch]);

  /* Counts for selected date */
  const counts = useMemo(() => {
    const c = { present: 0, absent: 0, late: 0, pending: 0 };
    if (!selectedDate) return c;
    filtered.forEach(s => { const st: StatusType = db_mem[s.studentId]?.[selectedDate] || 'pending'; c[st]++; });
    return c;
  }, [filtered, db_mem, selectedDate]);

  const pct = filtered.length > 0 ? Math.round(counts.present / filtered.length * 100) : 0;
          ...
          <View style={styles.markAllBtns}>
            {QUICK.map(q => (
              <TouchableOpacity
                ...
                activeOpacity={0.8}
              >
                <Ionicons name={STATUS_META[q.key].icon as any} size={24} color="#fff" />
                <Text style={styles.markAllBtnText}>All {STATUS_META[q.key].label}</Text>
              </TouchableOpacity>
            ))}
          </View>
          <TouchableOpacity style={[styles.markAllCancel, { backgroundColor: isDark ? '#334155' : '#f1f5f9' }]} onPress={() => setMarkAllVisible(false)}>
            <Text style={[styles.markAllCancelText, { color: isDark ? '#cbd5e1' : '#475569' }]}>Cancel</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
            ...
            {weekDays.map(day => {
              ...
              return (
                <TouchableOpacity
                  key={dStr}
                  disabled={blocked}
                  style={[styles.dateCell, { backgroundColor: bgColor, borderColor: bdrColor, opacity: isFuture ? 0.45 : 1 }]}
                ...
                >
                  ...
                  <Text style={[styles.dateCellNum, { color: isSel ? '#fff' : isSunday ? '#EF4444' : (isFuture ? theme.textSecondary : theme.text) }]}>
                    {day.getDate()}
                  </Text>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: scale(8) }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: scale(8) }}>
              <TouchableOpacity
                ...
                activeOpacity={0.8}
              >
                <Ionicons name="checkmark-done" size={16} color="#fff" style={{ marginRight: scale(4) }} />
                <Text style={{ color: '#fff', fontSize: scale(12), fontWeight: '700', letterSpacing: 0.3 }}>Mark All</Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity
                ...
                onPress={() => markAll('pending')}
                disabled={!selectedDate}
                activeOpacity={0.8}
              >
                <Ionicons name="refresh" size={16} color={isDark ? '#cbd5e1' : '#475569'} style={{ marginRight: scale(4) }} />
                <Text style={{ color: isDark ? '#cbd5e1' : '#475569', fontSize: scale(12), fontWeight: '700', letterSpacing: 0.3 }}>Clear</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  headerSub:   { fontSize: scale(10), color: 'rgba(255,255,255,0.75)', marginTop: scale(1) },
  ...
  dateCellBar: { position: 'absolute', bottom: 0, left: 0, height: scale(3), borderRadius: scale(1) },
  ...
  statSmall:  { fontSize: scale(8), fontWeight: '600', textTransform: 'uppercase', marginTop: scale(1) },
  ...
  actionIconBtn: { width: scale(36), height: scale(36), borderRadius: scale(8), justifyContent: 'center', alignItems: 'center' },
  ...
  row:        { flexDirection: 'row', alignItems: 'center', paddingHorizontal: scale(10), paddingVertical: scale(4), borderBottomWidth: 1 },
  rowLeft:    { flex: 1, flexDirection: 'row', alignItems: 'center' },
  serial:     { width: scale(22), fontSize: scale(10), textAlign: 'center' },
  avatar:     { width: scale(24), height: scale(24), borderRadius: scale(6), justifyContent: 'center', alignItems: 'center', marginLeft: scale(4) },
  avatarText: { fontSize: scale(10), fontWeight: '700' },
  info:       { flex: 1, marginLeft: scale(8) },
  name:       { fontSize: scale(11), fontWeight: '600' },
  meta:       { fontSize: scale(9), marginTop: scale(1) },

  quickBtns: { flexDirection: 'row', gap: scale(4) },
  qBtn:      { width: scale(26), height: scale(26), borderRadius: scale(6), borderWidth: 1, justifyContent: 'center', alignItems: 'center' },
  qBtnText:  { fontSize: scale(10), fontWeight: '800' },
  ...
  sheet:   { 
    ...
    maxHeight: '85%',
    elevation: 10,
    shadowColor: '#000', shadowOffset: { width: 0, height: scale(10) }, shadowOpacity: 0.25, shadowRadius: 15,
  },
  handle:  { width: scale(40), height: scale(5), borderRadius: scale(2.5), alignSelf: 'center', marginBottom: scale(16), opacity: 0.6 },
  ...
  histMeta:       { fontSize: scale(10), marginTop: scale(1) },
  ...
  histPillLabel:  { fontSize: scale(9),  fontWeight: '600', marginTop: scale(1) },
  calGrid:        { flexDirection: 'row', flexWrap: 'wrap', marginBottom: scale(10) },
  calCell:        { width: `${100 / 7}%` as any, alignItems: 'center', marginBottom: scale(8) },
  calDayLabel:    { fontSize: scale(9), fontWeight: '800', textTransform: 'uppercase', marginBottom: scale(4) },
  calDot:         { width: scale(34), height: scale(34), borderRadius: scale(17), justifyContent: 'center', alignItems: 'center', position: 'relative' },
  calDayNum:      { fontSize: scale(12), fontWeight: '600' },
  calStatusDot:   { position: 'absolute', bottom: scale(3), width: scale(5), height: scale(5), borderRadius: scale(2.5) },
  ...
  markAllTitle:      { fontSize: scale(18), fontWeight: '800', marginTop: scale(4), letterSpacing: -0.3 },
  markAllSub:        { fontSize: scale(12), marginBottom: scale(20), marginTop: scale(4) },
  markAllBtns:       { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', gap: scale(12), width: '100%', marginBottom: scale(16) },
  markAllBtn:        { width: '47%', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: scale(8), paddingVertical: scale(16), borderRadius: scale(16), elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: scale(2) }, shadowOpacity: 0.1, shadowRadius: 4 },
  markAllBtnText:    { fontSize: scale(13), fontWeight: '700', color: '#fff', letterSpacing: 0.3 },
  markAllCancel:     { width: '100%', paddingVertical: scale(14), borderRadius: scale(14), alignItems: 'center' },
  markAllCancelText: { fontSize: scale(14), fontWeight: '700' },
  ...
  rptMeta: { fontSize: scale(9), marginTop: scale(1) },
