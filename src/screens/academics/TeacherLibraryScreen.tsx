...
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, Dimensions, Animated, TextInput, StatusBar } from 'react-native';
...
export const LibraryScreen: React.FC = () => {
  ...
  const reduxCategories = useAppSelector((s: any) => s.appSettings?.libraryCategories) || [];
  const isLoading = useAppSelector((s: any) => s.appSettings?.loading === true);

  const [isSearching, setIsSearching] = React.useState(false);
  const [searchQuery, setSearchQuery] = React.useState('');

  const dynamicCategories = reduxCategories.filter((cat: any) => {
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      >
        <Ionicons name="arrow-back" size={scale(22)} color={isDark ? theme.text : '#ffffff'} />
            ...
            <TouchableOpacity style={[styles.searchBtn, { backgroundColor: isDark ? theme.border : 'rgba(255,255,255,0.2)' }]} onPress={() => setIsSearching(true)}>
              <Ionicons name="search" size={scale(18)} color={isDark ? theme.text : '#fff'} />
            </TouchableOpacity>
          </>
        ) : (
              ...
              return (
                <React.Fragment key={catName}>
                  {renderCard({
                    ...
                    color: cardColor,
                    onPress: () => isVideo
                      ? navigation.navigate('VideoGalleriesScreen')
                      : navigation.navigate('DocumentsScreen', { category: catName }),
                    count: isVideo
                      ? reduxGalleries.length
                      : notices.filter(n => n.category === catName).length,
                  })}
                </React.Fragment>
              );
            })}
