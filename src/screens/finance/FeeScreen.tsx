...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, TextInput, Modal, Alert, KeyboardAvoidingView, Platform, Animated, FlatList, ActivityIndicator , StatusBar} from 'react-native';
      ...
      <FlatList
        ...
        ListFooterComponent={
          visibleCount < filteredRecords.length ? (
            <TouchableOpacity
              ...
              style={{ alignItems: 'center', paddingVertical: scale(14), marginBottom: scale(10) }}
            >
              <Text style={{ fontSize: scale(13), fontWeight: '600', color: theme.primary }}>
                Load more ({filteredRecords.length - visibleCount} remaining)
              </Text>
            </TouchableOpacity>
          ) : filteredRecords.length > 0 ? (
            <Text style={{ textAlign: 'center', fontSize: scale(11), color: theme.textSecondary, paddingVertical: scale(10) }}>
          ...
          return (
            ...
            >
              ...
              <View style={[styles.trAvatar, { backgroundColor: statusColor + '18' }]}>
                <Text style={{ fontSize: scale(11), fontWeight: '700', color: statusColor }}>{initial}</Text>
              </View>

              {/* Name + roll + class */}
              <View style={{ flex: 1, marginLeft: scale(8) }}>
                <Text style={{ fontSize: scale(12), fontWeight: '600', color: theme.text }} numberOfLines={1}>{item.studentName}</Text>
                <Text style={{ fontSize: scale(10), color: theme.textSecondary }} numberOfLines={1}>{item.rollno} · {displayClass}</Text>
              </View>
        ...
        ListHeaderComponent={
          <View style={[styles.tableHeader, { backgroundColor: isDark ? theme.card : '#f8fafc', borderBottomColor: theme.border }]}>
            ...
            <Text style={[styles.thCell, { color: theme.textSecondary, flex: 1, marginLeft: scale(8) }]}>STUDENT</Text>
            <Text style={[styles.thCell, { color: theme.textSecondary, width: scale(58) }]}>STATUS</Text>
        ...
        <View style={{ flex: 1, backgroundColor: theme.background, paddingTop: StatusBar.currentHeight || 0 }}>
          ...
          >
            <TouchableOpacity
              ...
              style={{ width: scale(36), height: scale(36), borderRadius: scale(18), backgroundColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center' }}
            >
              <Ionicons name="close" size={22} color="#fff" />
            </TouchableOpacity>
            <View style={{ flex: 1, marginLeft: scale(12) }}>
              <Text style={{ fontSize: scale(17), fontWeight: '800', color: '#fff', letterSpacing: -0.3 }}>Edit Fee Record</Text>
              <Text style={{ fontSize: scale(11), color: 'rgba(255,255,255,0.8)', marginTop: scale(1) }}>
                {selectedRecord?.studentName || 'Student'}
              </Text>
            </View>
            <TouchableOpacity
              onPress={saveFeeRecord}
              style={{ width: scale(36), height: scale(36), borderRadius: scale(18), backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center' }}
            >
          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
            <ScrollView
              showsVerticalScrollIndicator={false}
              contentContainerStyle={{ padding: scale(12), paddingBottom: scale(24) }}
              keyboardShouldPersistTaps="handled"
            >
              {/* Student Info — compact 2-column read-only grid */}
              <Text style={{ fontSize: scale(9), fontWeight: '700', color: theme.textSecondary, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: scale(5), marginLeft: scale(2) }}>STUDENT INFO</Text>
              <View style={{ flexDirection: 'row', gap: scale(8), marginBottom: scale(12) }}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <View style={[feeStyles.roCell, { backgroundColor: theme.card, borderColor: theme.border, marginTop: scale(6) }]}>
              ...
              <Text style={{ fontSize: scale(9), fontWeight: '700', color: theme.textSecondary, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: scale(5), marginLeft: scale(2) }}>FEE DETAILS</Text>
              <View style={{ flexDirection: 'row', gap: scale(8), marginBottom: scale(12) }}>
                <View style={{ flex: 1, backgroundColor: theme.card, borderRadius: scale(10), borderWidth: 1, borderColor: theme.border, padding: scale(10) }}>
                  <Text style={{ fontSize: scale(9), fontWeight: '700', color: theme.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: scale(4) }}>Total Fee</Text>
                  <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <Text style={{ fontSize: scale(11), fontWeight: '800', color: '#10b981', marginRight: scale(3) }}>PKR</Text>
                    <TextInput
                      style={{ flex: 1, fontSize: scale(16), fontWeight: '800', color: theme.text, padding: 0 }}
                      value={totalFee}
                ...
                <View style={{ flex: 1, backgroundColor: theme.card, borderRadius: scale(10), borderWidth: 1, borderColor: theme.border, padding: scale(10) }}>
                  <Text style={{ fontSize: scale(9), fontWeight: '700', color: theme.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: scale(4) }}>Paid Amount</Text>
                  <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <Text style={{ fontSize: scale(11), fontWeight: '800', color: '#16a34a', marginRight: scale(3) }}>PKR</Text>
                    <TextInput
                      style={{ flex: 1, fontSize: scale(16), fontWeight: '800', color: theme.text, padding: 0 }}
                      value={paidAmount}
                      onChangeText={(v) => {
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: scale(5), marginTop: scale(2) }}>
                <Text style={{ fontSize: scale(9), fontWeight: '700', color: theme.textSecondary, textTransform: 'uppercase', letterSpacing: 0.8, marginLeft: scale(2) }}>
                  PAID MONTHS {new Date().getFullYear()}
                </Text>
                {feeMonth.length > 0 && (
                  <TouchableOpacity onPress={() => setFeeMonth([])}>
                    <Text style={{ fontSize: scale(9), fontWeight: '700', color: '#ef4444' }}>CLEAR</Text>
                  </TouchableOpacity>
                )}
              </View>
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: scale(6), marginBottom: scale(12) }}>
                {monthOptions.map((month) => {
                  ...
                  return (
                    ...
                    >
                      <Text style={{ fontSize: scale(10), fontWeight: '700', color: sel ? '#fff' : theme.text }}>
                        {month.slice(0, 3)}
                      </Text>
                    </TouchableOpacity>
                  );
              ...
              {(() => {
                ...
                return (
                  <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: `${clr}08`, borderWidth: 1, borderColor: `${clr}20`, borderRadius: scale(10), padding: scale(10), marginBottom: scale(14) }}>
                    <Ionicons name={isPaid ? 'checkmark-circle' : 'alert-circle'} size={18} color={clr} />
                    <View style={{ marginLeft: scale(8), flex: 1 }}>
                      <Text style={{ fontSize: scale(13), fontWeight: '800', color: clr }}>
                        {isPaid ? 'Fully Paid' : 'Payment Due'}
                      </Text>
                      <Text style={{ fontSize: scale(10), color: theme.textSecondary }}>
              ...
              >
                <TouchableOpacity
                  onPress={saveFeeRecord}
                  style={{ paddingVertical: scale(12), flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}
                >
                  <Ionicons name="checkmark-circle" size={17} color="#fff" />
                  <Text style={{ color: '#fff', fontSize: scale(14), fontWeight: '800', marginLeft: scale(7) }}>Save Changes</Text>
                </TouchableOpacity>
              </LinearGradient>

              {/* Fee History */}
              <View style={{ marginTop: scale(16) }}>
                <Text style={{ fontSize: scale(9), fontWeight: '700', color: theme.textSecondary, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: scale(8), marginLeft: scale(2) }}>
                  FEE HISTORY
                </Text>
                {historyLoading ? (
                  <View style={{ alignItems: 'center', paddingVertical: scale(12) }}>
                  ...
                  <View style={[feeStyles.historyEmpty, { backgroundColor: theme.card, borderColor: theme.border }]}>
                    ...
                    <Text style={{ fontSize: scale(11), color: theme.textSecondary, marginLeft: scale(8) }}>No previous payments</Text>
                  </View>
                ) : (
                  feeHistory.map((entry, idx) => (
                    <View key={idx} style={[feeStyles.historyRow, {
                      ...
                      <View style={{ flex: 1, marginLeft: scale(8) }}>
                        ...
                        {entry.months ? (
                          <Text style={{ fontSize: scale(9), color: theme.textSecondary }} numberOfLines={1}>
                            {entry.months}
                          </Text>
                        ) : null}
                      </View>
                      {/* Right: amount */}
                      <Text style={{ fontSize: scale(12), fontWeight: '800', color: '#10b981' }}>
...
const styles = StyleSheet.create({
  ...
  tableRow: {
    flexDirection: 'row', alignItems: 'center',
    paddingHorizontal: scale(12), paddingVertical: scale(9),
    borderBottomWidth: 0.5,
  },
  trCol: {},
  trSerial: { width: scale(22), fontSize: scale(10), textAlign: 'center', marginRight: scale(4) },
  trAvatar: {
  ...
  trStatusBadge: {
    ...
    borderRadius: scale(6), marginRight: scale(8), width: scale(58),
  },
  trDot: { width: scale(5), height: scale(5), borderRadius: scale(2.5), marginRight: scale(3) },
  trStatusText: { fontSize: scale(9), fontWeight: '700', textTransform: 'capitalize' },
  trAmount: { fontSize: scale(11), fontWeight: '700', width: scale(60), textAlign: 'right' },
  trEditBtn: { padding: scale(6), borderRadius: scale(6), marginLeft: scale(4) },
  ...
  sheetHandle: {
    ...
    alignSelf: 'center', marginBottom: scale(12),
  },
  sheetHeader: {
    flexDirection: 'row', alignItems: 'center',
    justifyContent: 'space-between', marginBottom: scale(14),
  },
  sheetTitle: { fontSize: scale(15), fontWeight: '700', letterSpacing: -0.2 },
  sheetBtn: {
    paddingHorizontal: scale(14), paddingVertical: scale(7),
    borderRadius: scale(8), minWidth: scale(52), alignItems: 'center',
  },
  monthGrid: {
    flexDirection: 'row',
    flexWrap: 'wrap',
