...
export const persistPushReadIds = async (readIds: string[]) => {
    try {
        await AsyncStorage.setItem('push_read_ids_teacher', JSON.stringify(readIds));
    } catch (e) {
...
export const initPushNotificationsListener = (
    ...
    // Load read state
    AsyncStorage.getItem('push_read_ids_teacher').then(stored => {
        if (stored) {
    ...
    return onSnapshot(
        ...
        (snapshot) => {
            ...
            // Client side filtering for matching role & institute/department
            fetched = fetched.filter(n => {
                // It must target Teachers or Both
                if (n.audience && n.audience !== 'Teachers' && n.audience !== 'Both' && n.targetRole !== 'Teachers' && n.targetRole !== 'Both') return false;
                
                // If it specifies institute, must match
                if (n.instituteId && n.instituteId !== profile?.instituteId) return false;

                // If it specifies department, must match
                if (n.departmentId && n.departmentId !== profile?.departmentId) return false;

                // If it specifies subject, must match (checking if teacher has this subject in array or matching single subject)
                if (n.subjectId && profile?.subjects && !profile.subjects.includes(n.subjectId)) return false;
